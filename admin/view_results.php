...
$i=1;
$q=mysql_query("select * from results left join departments on results.depart_id=departments.depart_id left join courses on results.subject_id=courses.course_id left join semester on results.semester_id=semester.semester_id left join staff on results.staff_id=staff.staff_id where results.depart_id='".$_SESSION['depart_id']."'");
...
<td><a href="department_login.php?page=show_result&rid=<?php echo $r['result_id'].'&flage='.$_GET['flage'];?>">Show Result Detail</a></td><td><a href="delete_result.php?result_id=<?php echo $r['result_id'];?>" onclick="return sure()">Delete</a></td>
</tr>
<?php
$i++;
}
